...
let etat_perso : perso -> string = fun perso ->
  ...
  let level_sup = make_ligne (String.length toucher) "" "  |" "Niveau supérieur" "" in
    ...
    (make_ligne reference debut fin level_sup (string_of_element  (niveau_superieur perso)) ) ^ delimitation^
    (make_ligne reference debut fin toucher (string_of_element  (chance_toucher perso)^"%") ) ^ delimitation^
    (make_ligne reference debut fin degat (string_of_element  (nb_degats perso)) ) ^ delimitation^
